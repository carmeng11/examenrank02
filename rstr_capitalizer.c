...
void	rstr_capitalizer(char *str)
{
	...
	while (str[i])
	{
		if ((str[i] >= 'A' && str[i] <= 'Z') && str[i + 1] != ' ' && str[i + 1] != '\0')// si pongo la comprobación del tab me lo hace mal&& str[i + 1] == '\t')) //ME CAMBIA A MINUSCULAS TODAS LAS LETRAS EXCEPTO LAS FINALES DE CADA PALABRA Y LA ÚLTIMA AL LLEGAAR AL CARACTER NULO
			str[i] += 32;
